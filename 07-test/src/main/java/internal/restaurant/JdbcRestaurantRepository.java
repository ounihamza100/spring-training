...
public class JdbcRestaurantRepository implements RestaurantRepository {
	 ...
	 */
	private Map<String, rewards.internal.restaurant.Restaurant> restaurantCache;
	...
	public rewards.internal.restaurant.Restaurant findByMerchantNumber(String merchantNumber) {
	...
	void populateRestaurantCache() {
		...
		restaurantCache = new HashMap<String, rewards.internal.restaurant.Restaurant>();
		...
		try {
			...
			rs = ps.executeQuery();
			while (rs.next()) {
				rewards.internal.restaurant.Restaurant restaurant = mapRestaurant(rs);
	 ...
	 */
	private rewards.internal.restaurant.Restaurant queryRestaurantCache(String merchantNumber) {
		rewards.internal.restaurant.Restaurant restaurant = restaurantCache.get(merchantNumber);
		if (restaurant == null) {
	 ...
	 */
	private rewards.internal.restaurant.Restaurant mapRestaurant(ResultSet rs) throws SQLException {
		...
		// map to the object
		rewards.internal.restaurant.Restaurant restaurant = new Restaurant(number, name);
