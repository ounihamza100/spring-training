...
public class JdbcAccountRepository implements AccountRepository {
	...
	public rewards.internal.account.Account findByCreditCard(String creditCardNumber) {
		...
		rewards.internal.account.Account account = null;
	...
	public void updateBeneficiaries(rewards.internal.account.Account account) {
		...
		try {
			conn = dataSource.getConnection();
			ps = conn.prepareStatement(sql);
			for (rewards.internal.account.Beneficiary beneficiary : account.getBeneficiaries()) {
	 ...
	 */
	private rewards.internal.account.Account mapAccount(ResultSet rs) throws SQLException {
		rewards.internal.account.Account account = null;
		while (rs.next()) {
			if (account == null) {
	 ...
	 * @throws SQLException an exception occurred extracting data from the result set
	 */
	private rewards.internal.account.Beneficiary mapBeneficiary(ResultSet rs) throws SQLException {
